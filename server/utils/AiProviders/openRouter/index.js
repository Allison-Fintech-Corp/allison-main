...
class OpenRouterLLM {
  constructor(embedder = null, modelPreference = null) {
    ...
    // Allow default temperature to be set via env; fallback to 0.7
    const parsedTemp = parseFloat(process.env.LLM_DEFAULT_TEMPERATURE);
    this.defaultTemp = isNaN(parsedTemp) ? 0.7 : Math.min(Math.max(parsedTemp, 0), 2);
  ...
  async getChatCompletion(
    messages = null,
    { temperature = this.defaultTemp, maxTokens = Number(process.env.OPENROUTER_MAX_TOKENS) || undefined } = {}
  ) {
